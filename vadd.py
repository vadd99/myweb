...
# --- PERBAIKAN KRITIS PADA DECORATOR ADMIN ---
def admin_required(f):
    @wraps(f)
    def decorated_function(*args, **kwargs):
        if not session.get('is_admin'):
            # Jika ini adalah panggilan API, kirim error JSON
            if request.path.startswith('/api/'):
                return jsonify({'error': 'Admin access required'}), 403 # 403 Forbidden
            # Jika bukan, redirect ke halaman login admin
...
# --- RUTE HALAMAN & API (Tidak ada perubahan di sini) ---
@app.route('/')
def home():
...
@admin_required
def update_balance():
    data = request.get_json(); user_id, amount = data.get('user_id'), data.get('amount'); db = get_db()
    db.execute("UPDATE users SET balance = balance + ? WHERE id = ?", (amount, user_id)); db.commit()
...
def admin_update_user():
    data = request.get_json(); user_id, new_password = data.get('id'), data.get('new_password'); db = get_db()
    if new_password:
